...
const FIRMS_BOX = "101.30,0.40,101.60,0.70";
...
// Kode wilayah (adm4) per kecamatan
...
// Satu titik koordinat wakil per kecamatan
...
function normalisasiFfmcCategory(raw) {
  ...
  if (["sangat tinggi", "very high"].includes(v)) {
    return "Sangat Tinggi";
  }

  if (["tinggi", "high"].includes(v)) {
    return "Tinggi";
  }

  if (["sedang", "moderate", "medium"].includes(v)) {
    return "Sedang";
  }

  if (["rendah", "low"].includes(v)) {
    return "Rendah";
  }
 ...
 *
 * Prinsip:
 * - Tidak membuat threshold suhu/RH/angin sendiri.
 * - Hotspot NASA FIRMS adalah sinyal observasi.
 * - Hotspot yang muncul kembali pada run berikutnya menjadi sinyal persistensi.
 * - BMKG suhu/RH/angin menjadi konteks meteorologis.
 * - FFMC BMKG digunakan bila kategori terstruktur tersedia.
 *
 * Saat FFMC belum tersedia:
 *   hotspot pertama       -> WASPADA
 *   hotspot berulang      -> SIAGA
 *   tidak ada hotspot     -> NORMAL
 */
function hitungStatusEws({ hotspot, persistent, ffmcCategory }) {
  ...
  // Jika FFMC kategori tersedia, gunakan sebagai penguat.
  if (ffmc) {
    if (
      persistent &&
      ffmc === "Sangat Tinggi"
    ...
    if (
      (adaHotspot && ffmcTinggi) ||
      (persistent && adaHotspot)
    ) {
      return "SIAGA";
    }
  ...
  // Jika FFMC belum tersedia:
  // jangan menebak dari warna peta BMKG.
  if (
    persistent &&
    adaHotspot
  ) {
...
function alasanStatusEws({
  ...
  if (Number(hotspot || 0) > 0) {
    alasan.push(
      `Hotspot NASA FIRMS terdeteksi (${hotspot})`
    );
  } else {
    alasan.push(
      "Tidak ada hotspot NASA FIRMS pada data yang diperiksa"
    );
  }

  if (persistent) {
    alasan.push(
      "Hotspot terdeteksi berulang pada kecamatan ini"
    );
  }

  if (ffmc) {
    alasan.push(
      `FFMC BMKG: kategori ${ffmc}`
    );
  } else {
    alasan.push(
      "Kategori FFMC BMKG belum tersedia sebagai data terstruktur"
    );
  }
...
async function ambilCuacaTerkini(adm4) {
  const res = await fetch(
    `${BMKG_ENDPOINT}?adm4=${encodeURIComponent(adm4)}`
  );

  if (!res.ok) {
    throw new Error("BMKG HTTP " + res.status);
  }
...
async function bacaBatasKecamatan() {
  try {
    ...
    const match = src.match(
      /const\s+BATAS_KECAMATAN_PEKANBARU\s*=\s*(\{[\s\S]*\})\s*;\s*$/
    );
    ...
    console.warn(
      "[Wilayah] Batas kecamatan tidak dapat dimuat, fallback centroid dipakai:",
      e.message
    );
...
function pointInGeometry(lng, lat, geometry) {
  ...
  if (geometry.type === "MultiPolygon") {
    return geometry.coordinates.some(
      poly => pointInPolygon(lng, lat, poly)
    );
  }
...
function klasifikasiKecamatan(lng, lat, batas) {
  if (batas?.features) {
    const diKota = batas.features.some(
      f => pointInGeometry(
        lng,
        lat,
        f.geometry
  ...
  for (
    const [kec, titik]
    of Object.entries(TITIK_ACUAN_KECAMATAN)
  ) {
...
function dedupeFirms(points) {
  ...
  for (const p of points) {
    ...
    if (
      !old ||
      normalisasiConfidenceFirms(p.confidence) >
        normalisasiConfidenceFirms(old.confidence)
    ) {
      seen.set(key, p);
    }
  }
...
async function ambilTitikFirms() {
  if (!FIRMS_MAP_KEY) {
    console.warn(
      "[FIRMS] FIRMS_MAP_KEY tidak diset (secret kosong), lewati pengambilan hotspot."
    );
  ...
  for (const sensor of sumber) {
    try {
      const url =
        ...
        `${FIRMS_MAP_KEY}/${sensor}/${FIRMS_BOX}/1`;
      ...
      if (!/^latitude,longitude/i.test(csv)) {
        throw new Error(
          "respons bukan CSV FIRMS"
        );
      }
      ...
      for (const r of baris.slice(1)) {
        ...
        const kec = klasifikasiKecamatan(
          lng,
          lat,
          batas
        );
        ...
        obj._confidenceScore =
          normalisasiConfidenceFirms(
            obj.confidence
          );

        // Low-confidence tidak dijadikan sinyal utama EWS.
        if (obj._confidenceScore < 2) {
          continue;
        }
      ...
      console.log(
        `[FIRMS] ${sensor}: ${baris.length - 1} record diterima sebelum filter.`
      );
    } catch (e) {
      console.warn(
        `[FIRMS] ${sensor} gagal:`,
...
function kelompokkanDetailHotspot(titikFirms) {
  const hasil = {};
...
function waktuHotspotWib(hotspot) {
  if (!hotspot?.acq_date) {
    return (
...
async function cariKelurahan(lat, lng) {
  if (
    !Number.isFinite(lat) ||
    !Number.isFinite(lng)
  ) {
  ...
  try {
    ...
    const res = await fetch(
      url,
      {
        headers: {
          "User-Agent":
            "FireSentry-Karhutla-EWS/1.0 (BPBD Pekanbaru)"
        }
      }
    );
    ...
    console.warn(
      "[Wilayah] Reverse geocoding kelurahan gagal:",
      e.message
    );
...
async function supabaseRequest(
  ...
  if (!res.ok) {
    throw new Error(
      `Supabase HTTP ${res.status}: ${await res.text()}`
    );
  }
...
async function simpanKeSupabase(
  ...
  const rows = Object.entries(statusBaru)
    ...
    .map(([kecamatan, v]) => ({
      kecamatan,
      risiko: v.risiko,
      status_ews: v.statusEws || null,
      suhu: v.suhu,
      kelembapan: v.kelembapan,
      angin_kecepatan:
        v.anginKecepatan ?? null,
      angin_arah:
        v.anginArah ?? null,
      hotspot: v.hotspot,
      waktu_cek: v.waktuCek,
      sumber:
        "BMKG + NASA FIRMS",
      ...
      fdrs_fwi:
        v.fdrs?.fwi ?? null,
      fdrs_numeric_available:
        !!v.fdrs?.numericValuesAvailable,
      fdrs_source:
        v.fdrs?.source ??
        "BMKG SPARTAN"
    }));
  ...
  if (rows.length) {
    await supabaseRequest(
      "ews_status_current?on_conflict=kecamatan",
      {
        method: "POST",
        headers: {
          Prefer:
  ...
  const gagal =
    kecamatanGagal.length ===
    Object.keys(KECAMATAN_ADM4).length
      ? "gagal"
      : kecamatanGagal.length
        ? "sebagian"
        : "berhasil";
...
async function kirimTelegram(pesan) {
  if (
    ...
    console.warn(
      "[Telegram] TELEGRAM_BOT_TOKEN/TELEGRAM_CHAT_ID belum diset di GitHub Secrets, notifikasi dilewati."
    );
...
async function susunPesan(
  kecamatan,
  statusEws,
  detail
) {
  const emoji =
    statusEws === "PERINGATAN"
      ? "🔴"
      : statusEws === "SIAGA"
        ? "🟠"
        : statusEws === "WASPADA"
  ...
  const mapUrl =
    hotspot
      ? `https://rrencuyy.github.io/firesentry_karhutla_pku/peta.html?fokus=hotspot&lat=${encodeURIComponent(hotspot.latitude)}&lng=${encodeURIComponent(hotspot.longitude)}&kecamatan=${encodeURIComponent(kecamatan)}&kelurahan=${encodeURIComponent(kelurahan)}`
      : "https://rrencuyy.github.io/firesentry_karhutla_pku/peta.html";
  ...
  return (
    ...
    `Suhu: <b>${detail?.suhu ?? "-"}°C</b> | Kelembapan: <b>${detail?.kelembapan ?? "-"}%</b>\n` +
    `Titik panas: <b>${detail?.hotspot ?? 0}</b>\n` +
    `Waktu: ${waktu}\n\n` +
    `latitude &amp; longitude: <b>${lat}, ${lng}</b>\n\n` +
    `🔗 <a href="${mapUrl}">Lihat peta kejadian</a>`
  );
}

/**
 * Alert KHUSUS untuk masalah SISTEM
 * — dipakai saat BMKG gagal total atau script error,
 * supaya petugas/dev tahu sistemnya sedang "buta",
 * bukan diam-diam berhenti kerja.
 */
async function kirimAlertSistem(pesan) {
  await kirimTelegram(
    `⚠️ <b>PERINGATAN SISTEM FIRESENTRY</b>\n` +
    `${pesan}\n\n` +
...
// Kompatibel dengan format LAMA dan format BARU.
function risikoDariEntriLama(entri) {
  if (!entri) {
    return "Rendah";
  }

  if (typeof entri === "string") {
    return entri;
  }
...
async function main() {
  ...
  const statusLama =
    await bacaStatusLama();

  const fdrs =
    await bacaFdrsMetadata();

  const statusBaru = {};
  const dinotifikasi = [];

  const riwayatEskalasi =
    Array.isArray(
      statusLama?._meta
        ?.riwayatEskalasi
    )
  ...
  const titikFirms =
    await ambilTitikFirms()
      .catch(e => {
        console.warn(
          "[FIRMS] Gagal ambil hotspot:",
          e.message
        );
  ...
  const hotspotPerKecamatan =
    kelompokkanHotspot(
      titikFirms
    );

  const hotspotDetailPerKecamatan =
    kelompokkanDetailHotspot(
      titikFirms
    );
  ...
  for (
    ...
    try {
      cuaca =
        await ambilCuacaTerkini(
          adm4
        );
    } catch (e) {
      console.warn(
        `[BMKG] Gagal ambil cuaca untuk ${kecamatan}:`,
        e.message
      );
    }
    ...
    /*
     * PERBAIKAN PENTING:
     * Ambil status lama terlebih dahulu supaya
     * persistensi hotspot bisa dihitung.
     */
    const entriLama =
      statusLama[kecamatan];
    ...
    statusBaru[kecamatan] = {
      ...
      anginKecepatan:
        cuaca.anginKecepatan,

      anginArah:
        cuaca.anginArah,

      waktuPrakiraan:
        cuaca.waktuPrakiraan,
      ...
      fdrs:
        fdrs
          ? {
              ...
              numericValuesAvailable:
                !!fdrs.numericValuesAvailable,
    ...
    /*
     * Jika status lama sudah menyimpan statusEws,
     * gunakan langsung.
     *
     * Jika format lama hanya menyimpan risiko:
     * Rendah  -> NORMAL
     * Sedang  -> WASPADA
     * Tinggi  -> SIAGA
     */
    ...
    console.log(
      ...
      `suhu=${cuaca.suhu} ` +
      `RH=${cuaca.kelembapan} ` +
      `angin=${cuaca.anginKecepatan ?? "-"} km/h ` +
      `hotspot=${hotspot} ` +
  ...
  const riwayatTerpangkas =
    riwayatEskalasi.slice(
      0,
      MAKS_RIWAYAT_ESKALASI
    );
  ...
  statusBaru._meta = {
    ...
    sumber:
      "GitHub Actions terjadwal (BMKG + NASA FIRMS + BMKG FDRS + IQAir terpisah)",
  ...
  await simpanStatusBaru(
    statusBaru
  );

  try {
    await simpanKeSupabase(
      statusBaru,
      riwayatEskalasi,
      mulaiPada,
      sekarangIso,
    ...
    console.warn(
      "[Supabase] Penyimpanan database gagal, status JSON tetap tersimpan:",
      e.message
    );
  }
  ...
  if (
    ...
    console.error(
      "[ALERT] BMKG gagal untuk SEMUA kecamatan — sistem tidak bisa menilai risiko saat ini."
    );
    ...
    kecamatanGagal.length > 0
  ) {
    console.warn(
      `[ALERT] BMKG gagal untuk ${kecamatanGagal.length} dari ${totalKecamatan} kecamatan:`,
      kecamatanGagal.join(", ")
    );
  ...
  if (dinotifikasi.length) {
    console.log(
      "Notifikasi terkirim untuk:",
      dinotifikasi.join(", ")
    );
  } else {
    console.log(
      "Tidak ada eskalasi status — tidak ada notifikasi yang dikirim kali ini."
    );
  }
}
